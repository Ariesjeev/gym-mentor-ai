                ...
                try:
                    ...
                    if context and hasattr(context, "state") and context.state:
                        if getattr(context.state, "playing", False):
                            st.success("🎥 **The camera is live! (0-Second Delay Super-Fast Mode)**")
                        elif getattr(context.state, "signalling", False):
                            st.warning("⏳ **The camera is connecting... (Connecting...)** — If it takes more than 10 seconds, please check the browser permissions (Camera Permission) or refresh the page!")
                        else:
                            st.info("💡 **The camera is off.** Click the **`START`** button on the bottom left of the video box and allow the browser to use the camera!")

                    sync_metrics_update(context)
                except Exception as e:
                    st.error(f"🚨 **The camera failed to open (WebRTC Exception):**\n\n**Error Details:** `{e}`")
                    st.exception(e)
